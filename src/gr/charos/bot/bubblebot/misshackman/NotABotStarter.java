 ...
 */
public class NotABotStarter {

    private Random random;

    private NotABotStarter() {
    ...
    public Move doMove(AbstractBotState state) {
        ...
        if (validMoveTypes.size() <= 0) {
            return new BombableMove(); // No valid moves, pass
        }
        ...
        //TODO: fix this
        BomberPlayer me = null; 
        		//state.getPlayers().get(state.getMyName());
        ...
        return new BombableMove(randomMoveType, bombTicks); // Drop bomb if available
    }
